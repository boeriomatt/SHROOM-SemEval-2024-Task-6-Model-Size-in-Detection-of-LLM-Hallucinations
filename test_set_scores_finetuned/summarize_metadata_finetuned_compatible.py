...
def _first_present(mapping: dict, keys: list[str]):
    """Return the first non-None value found for any key in a flat mapping."""
    for key in keys:
...
def normalize_metadata_schema(data: dict) -> dict:
    ...
    acc = _first_present(
        scores,
        ["agnostic_acc", "acc_agnostic", "accuracy", "test_acc", "direct_acc", "acc"],
    )
    rho = _first_present(
        scores,
        ["agnostic_rho", "rho_agnostic", "rho", "test_rho", "direct_rho"],
    )
    ...
    if acc is not None:
        scores["agnostic_acc"] = acc
        scores["acc_agnostic"] = acc
    if rho is not None:
        scores["agnostic_rho"] = rho
    ...
    if not normalized.get("archive_prediction_path"):
        normalized["archive_prediction_path"] = _first_present(
            prediction_paths,
            ["archive_predictions", "run_predictions", "current_predictions"],
        )
...
def load_metadata_files(metadata_dir: Path) -> list[dict]:
    ...
    for path in metadata_dir.glob("*.json"):
        ...
        data = normalize_metadata_schema(raw_data)
        ...
        family = data.get("family_override") or infer_family(model_type, model_name)
        comparison_bucket = data.get("comparison_bucket_override") or infer_comparison_bucket(model_type, model_name)
    ...
    for bucket, bucket_rows in bucket_groups.items():
        ...
        for row in bucket_rows:
            ...
            if comparable:
                ...
                if not (len(pred_curr) == len(pred_prev) == len(ref_items)):
                    print(
                        f"Skipping significance for {previous['model_name']} -> {row['model_name']}: "
                        f"prediction/reference length mismatch "
                        f"(prev={len(pred_prev)}, curr={len(pred_curr)}, ref={len(ref_items)})"
                    )
                    previous = row
                    continue
