...
FINETUNED_DIR = Path("outputs/finetuned/flan_lora")
...
class ShroomPromptDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        prompt = support_prompt(str(ex["context"]), str(ex["hyp"]))
...
def encode_target_variants(tokenizer, variants: list[str], device: torch.device) -> list[torch.Tensor]:
    ...
    for variant in variants:
        ids = tokenizer(variant, return_tensors="pt").input_ids.to(device)
        if ids.ndim != 2 or ids.shape[0] != 1:
            raise ValueError(f"Unexpected target encoding shape for {variant!r}: {tuple(ids.shape)}")
...
def sequence_logprob_for_target(
    ...
    """Return differentiable per-example log P(target | prompt)."""
    batch_size = int(model_inputs["input_ids"].shape[0])
    labels = repeat_target_ids(target_ids, batch_size)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-name", required=True, help="HF FLAN-T5 model id or local checkpoint path.")
    parser.add_argument(
        "--train-path",
    ...
    parser.add_argument(
        "--lora-target-modules",
        default="q,v",
        help="Comma-separated target module names for T5 attention projections. Default: q,v",
    )
...
def main() -> None:
    ...
    base_model = build_base_model(args.model_name, device=device, use_amp=use_amp)
    ...
    yes_target_variants = encode_target_variants(tokenizer, yes_variants, device)
    no_target_variants = encode_target_variants(tokenizer, no_variants, device)
    ...
    print("\nStarting FLAN LoRA fine-tuning...")
    global_step = 0
    ...
    if checkpoint_dir_for_eval.exists():
        if final_eval_only:
            ...
            print(f"\nReloading best LoRA adapter checkpoint: {checkpoint_dir_for_eval}")
            import gc
    ...
    score_path = SCORES_DIR / f"flan_lora_scores__{args.label_mode}__{safe_model}__{run_tag}.txt"
    metadata_path = METADATA_DIR / f"run__flan_lora__finetuned__{args.label_mode}__{safe_model}__{run_tag}.json"
    ...
    if args.run_participant_scorer:
        submission_dir = PRED_SUBMISSIONS_DIR / f"flan_lora__{safe_model}__{run_tag}"
    ...
    loss_name = (
        "BCEWithLogitsLoss_soft_verbalizer_p_hallucination"
        if args.label_mode == "soft"
        else "CrossEntropyLoss_hard_verbalizer_label"
    )

    metadata = {
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "model_type": "flan_lora_finetuned_seq2seq_verbalizer",
