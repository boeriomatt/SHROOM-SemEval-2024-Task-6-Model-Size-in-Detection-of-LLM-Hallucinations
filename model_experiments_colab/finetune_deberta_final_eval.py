...
This version supports two training objectives and a final-evaluation mode:
...
Typical dev-split run:
    python finetune_deberta_final_eval.py \
...
Typical final test-set run, after hyperparameters are frozen:
    python finetune_deberta_final_eval.py \
      --model-name cross-encoder/nli-deberta-v3-base \
      --train-path data/SHROOM_dev-v2/val.model-agnostic.json \
      --eval-path data/SHROOM_test-labeled/test.model-agnostic.json \
      --epochs 2 --learning-rate 1e-5 \
      --label-mode soft --final-eval-only --fp16
"""
...
def predict_examples_timed(
    ...
    for i, ex in enumerate(examples, start=1):
        ...
        pred = {"label": label, "p(Hallucination)": p_hall}
        if include_id and "id" in ex:
            pred["id"] = ex["id"]
...
def run_checker_and_scorer(
    pred_current_dir: Path,
    score_path: Path,
    reference_dir: Path,
    is_val: bool,
) -> dict[str, float | str]:
    print("\nRunning participant output checker...")
    checker_cmd = [sys.executable, str(CHECKER_PATH), str(pred_current_dir)]
    if is_val:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "For final holdout evaluation: train for the fixed number of epochs without "
            "evaluating/selecting checkpoints during training, then evaluate once on eval-path. "
            "Use this when --eval-path points to the held-out test set."
        ),
    )
    ...
    parser.add_argument("--write-current", action="store_true", help="Also write outputs/predictions/current/<eval_file_stem>.json.")
    parser.add_argument(
        "--run-participant-scorer",
        action="store_true",
        help="Run participant checker/scorer on the final prediction file.",
    )
    parser.add_argument(
        "--score-split",
        choices=["auto", "val", "test"],
        default="auto",
        help="Participant-kit split mode. 'auto' infers val/test from the eval file name.",
    )
...
def main() -> None:
    ...
    checkpoint_dir = run_dir / ("checkpoint-final" if args.final_eval_only else "checkpoint-best")
    ...
    print(f"Final eval only: {args.final_eval_only}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.final_eval_only:
            ...
            print(
                ...
                "eval_skipped=True (final_eval_only)"
            )
        else:
            ...
            if is_better(eval_metrics, best_metrics, args.best_metric):
                ...
                print(f"Saved new best checkpoint to: {checkpoint_dir}")
    ...
    if args.final_eval_only:
        ...
        model.save_pretrained(checkpoint_dir)
        tokenizer.save_pretrained(checkpoint_dir)
        print(f"Saved final fixed-epoch checkpoint to: {checkpoint_dir}")

    if checkpoint_dir.exists():
        reload_label = "final fixed-epoch checkpoint" if args.final_eval_only else "best checkpoint"
        print(f"\nReloading {reload_label}: {checkpoint_dir}")
    ...
    if args.score_split == "auto":
        score_split = "test" if eval_file_stem.startswith("test.") else "val"
    else:
        score_split = args.score_split
    is_val_score = score_split == "val"
    ...
    participant_submission_dir = run_dir / "participant_submission"
    if args.write_current or args.run_participant_scorer:
        participant_submission_dir.mkdir(parents=True, exist_ok=True)
        current_pred_path = participant_submission_dir / f"{eval_file_stem}.json"
        with current_pred_path.open("w", encoding="utf-8") as f:
            json.dump(predictions, f, ensure_ascii=False, indent=2)
        print(f"Saved participant-format predictions to: {current_pred_path}")

    if args.run_participant_scorer:
        participant_scores = run_checker_and_scorer(
            pred_current_dir=participant_submission_dir,
    ...
    metadata = {
        ...
        "checkpoint_selection": "final_fixed_epoch_no_eval_selection" if args.final_eval_only else f"best_{args.best_metric}",
        "seed": args.seed,
