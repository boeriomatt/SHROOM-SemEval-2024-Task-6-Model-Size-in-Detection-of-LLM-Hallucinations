...
model_experiments project root. It mirrors the Gemma LoRA fine-tuning flow while
using Qwen Instruct causal-LM chat prompting and Qwen-compatible LoRA target
module names.
...
Default local validation candidate:
    - Qwen-compatible attention LoRA targets: q_proj,k_proj,v_proj,o_proj
    - r=16, alpha=32, dropout=0.05
    - soft labels, learning_rate=2e-4, epochs=4
    - effective train batch size 4 via batch_size=1 and grad_accum_steps=4
    - fp32 by default; add --fp16/--bf16 only after checking stability
...
Final test-style run after hyperparameters are fixed:
    python finetune_qwen_lora_final_eval.py \
      --model-name Qwen/Qwen2.5-0.5B-Instruct \
      --train-path data/SHROOM_dev-v2/val.model-agnostic.json \
...
FINETUNED_DIR = Path("outputs/finetuned/qwen_lora")
...
YES_VARIANTS_DEFAULT = "yes"
NO_VARIANTS_DEFAULT = "no"
...
def assert_finite_tensor(name: str, tensor: torch.Tensor) -> None:
    ...
    if not torch.isfinite(tensor).all():
        ...
        raise FloatingPointError(
            ...
            f"Sample values: {sample}. For Qwen, first retry with --bf16 or without --fp16."
        )
...
def causal_sequence_logprob_for_target(
    ...
    # This is especially important for Qwen fp16 runs, where verbalizer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Final test-set fine-tune/evaluate Qwen Instruct models on SHROOM with LoRA verbalizer scoring.")
    parser.add_argument("--model-name", required=True, help="HF Qwen instruction model id or local checkpoint path.")
    parser.add_argument(
        "--train-path",
    ...
    # Qwen / causal-LM loading settings
    ...
    # LoRA settings. Qwen attention projection modules match the Gemma-style causal-LM setup.
    ...
    parser.add_argument(
        ...
        help=(
            "Comma-separated target module names for Qwen. Default: q_proj,k_proj,v_proj,o_proj. "
...
def main() -> None:
    ...
    if args.fp16 and requested_device.type == "cuda":
        print(
            "Warning: If Qwen fp16 becomes unstable, try --bf16 on supported GPUs or remove --fp16. "
    ...
    if checkpoint_dir_for_eval.exists():
        if final_eval_only:
            ...
            # in-memory PEFT model avoids reloading a second full Qwen base model,
            # which helps preserve Colab memory headroom for larger rungs.
            try:
                del optimizer
            except NameError:
                pass
    ...
    score_path = SCORES_DIR / f"qwen_lora_scores__{args.label_mode}__{safe_model}__{run_tag}.txt"
    metadata_path = METADATA_DIR / f"run__qwen_lora__finetuned__{args.label_mode}__{safe_model}__{run_tag}.json"
    ...
    if args.run_participant_scorer:
        submission_dir = PRED_SUBMISSIONS_DIR / f"qwen_lora__{safe_model}__{run_tag}"
    ...
    metadata = {
        ...
        "model_type": "qwen_lora_finetuned_causal_lm_verbalizer",
